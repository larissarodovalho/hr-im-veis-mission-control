...
CREATE INDEX IF NOT EXISTS imovel_link_eventos_link_tipo_idx ON public.imovel_link_eventos (link_id, tipo_evento);
CREATE INDEX IF NOT EXISTS imovel_links_created_idx ON public.imovel_links_compartilhados (created_at DESC);
CREATE INDEX IF NOT EXISTS imovel_links_corretor_idx ON public.imovel_links_compartilhados (corretor_id);

CREATE OR REPLACE FUNCTION public.imovel_links_performance(
  _inicio timestamptz,
  _fim timestamptz,
  _corretor uuid DEFAULT NULL,
  _imovel text DEFAULT NULL,
  _conta uuid DEFAULT NULL,
  _oportunidade uuid DEFAULT NULL,
...
BEGIN
  ...
  WITH base AS (
    ...
    WHERE l.created_at >= _inicio AND l.created_at <= _fim
      ...
      AND (_imovel IS NULL OR EXISTS (
            SELECT 1 FROM public.imovel_link_itens it
  ...
  ev AS (
    ...
    FROM public.imovel_link_eventos e
    JOIN base b ON b.id = e.link_id
    WHERE COALESCE((e.metadata->>'bot')::boolean, false) = false
      AND COALESCE((e.metadata->>'invalidado')::boolean, false) = false
  ),
  agg AS (
    SELECT b.id,
      b.corretor_id, b.tipo, b.validade_minutos, b.conta_id, b.oportunidade_id,
    ...
    FROM base b
    LEFT JOIN ev e ON e.link_id = b.id
  ...
  kpis AS (
    SELECT
      ...
      (SELECT COUNT(DISTINCT v.id) FROM public.vendas v
        WHERE v.oportunidade_id IN (SELECT oportunidade_id FROM f WHERE oportunidade_id IS NOT NULL))::int AS vendas,
  ...
  por_corretor AS (
    SELECT COALESCE(p.nome,'Sem responsável') AS chave, COUNT(*)::int AS gerados,
      ...
      COALESCE(SUM(f.ev_gostei),0)::int AS gostei,
      COALESCE(SUM(f.ev_visita),0)::int AS visitas
  ...
  itens AS (
    ...
    FROM f JOIN public.imovel_link_itens it ON it.link_id = f.id
    JOIN public.imoveis i ON i.id = it.imovel_id
  ),
  por_imovel AS (
...
REVOKE ALL ON FUNCTION public.imovel_links_performance(timestamptz,timestamptz,uuid,text,uuid,uuid,text,text,text,text,text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.imovel_links_performance(timestamptz,timestamptz,uuid,text,uuid,uuid,text,text,text,text,text) TO authenticated;
REVOKE ALL ON FUNCTION public.criar_notificacao(uuid,text,text,text,uuid,uuid,uuid,uuid,text,text) FROM PUBLIC, anon;
