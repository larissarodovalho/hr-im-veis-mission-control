...
export default function AuthPage() {
  ...
  async function loginGoogle() {
    ...
    try {
      ...
      // Se redirected, navegador redireciona; senão sessão já está pronta
    } finally {
      setBusy(false);
    }
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background via-background to-primary/5 p-4">
      <Card className="w-full max-w-md border-border/50 shadow-2xl">
        <CardHeader className="text-center space-y-3">
          <div className="mx-auto w-16 h-16 rounded-full bg-white flex items-center justify-center overflow-hidden">
            <img src={hrLogo} alt="HR Imóveis" className="w-12 h-12 object-contain" />
          </div>
          <div>
            <CardTitle className="text-2xl font-display">HR Imóveis</CardTitle>
            <CardDescription>Mission Control · CRM Comercial</CardDescription>
          </div>
        </CardHeader>
        <CardContent>
          ...
          <Button variant="outline" className="w-full" onClick={loginGoogle} disabled={busy}>
