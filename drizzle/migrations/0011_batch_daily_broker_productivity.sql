  ...
  aval AS MATERIALIZED (
    SELECT g.*,
      ...
      (g.ativa AND (g.tarefa_vencida OR g.ultima_interacao_em IS NULL
        ...
        OR (g.proxima_acao_calculada IS NOT NULL AND (g.proxima_acao_calculada AT TIME ZONE 'America/Cuiaba')::date<=g.dia))) exigivel,
      ...
      CASE
        ...
        WHEN (g.updated_at AT TIME ZONE 'America/Cuiaba')::date<=g.dia AND (g.motivo_desclassificacao IS NOT NULL OR g.qualificacao_status='desqualificado') THEN 'desqualificado'
        WHEN (g.updated_at AT TIME ZONE 'America/Cuiaba')::date<=g.dia AND g.qualificacao_status IN ('sem_interesse','sem_retorno','encerrado') THEN g.qualificacao_status
