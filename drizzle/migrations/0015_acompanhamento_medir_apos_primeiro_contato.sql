CREATE OR REPLACE FUNCTION public.acompanhamento_apurar_diario(
  ...
  _origens_carteira text[] DEFAULT ARRAY['base_hr','marketing','carteira_propria']
) RETURNS jsonb
LANGUAGE plpgsql
VOLATILE
SECURITY DEFINER
...
BEGIN
  ...
  primeiro_contato AS MATERIALIZED (
    SELECT i.conta_id,(min(i.created_at) AT TIME ZONE 'America/Cuiaba')::date dia_primeiro_contato
  ...
  contas_base AS MATERIALIZED (
    SELECT c.*,pa.atribuida_em primeira_atribuicao_em,pa.dono_original_id,
      cd.desfecho_tipo,
      coalesce(pc.dia_primeiro_contato,
        CASE WHEN c.etapa_funil IS DISTINCT FROM 'a_contatar'
          THEN (c.updated_at AT TIME ZONE 'America/Cuiaba')::date END) dia_primeiro_contato,
    ...
    LEFT JOIN primeiro_contato pc ON pc.conta_id=c.id
    WHERE (c.created_at AT TIME ZONE 'America/Cuiaba')::date<=v_fim
  ),
  ...
  grade_base AS MATERIALIZED (
    ...
    JOIN contas_base c ON (c.created_at AT TIME ZONE 'America/Cuiaba')::date<=d.dia AND c.origem_calculada=ANY(v_origens)
      AND c.dia_primeiro_contato IS NOT NULL AND d.dia >= c.dia_primeiro_contato
